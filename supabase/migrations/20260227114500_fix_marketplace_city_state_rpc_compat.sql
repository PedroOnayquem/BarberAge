-- Compatibility hardening for marketplace city/state filtering.
-- Keeps RPC available even when environments diverge between `shops` and `barbershops`.
...
begin
  if to_regclass('public.shops') is not null then
    execute 'create index if not exists idx_shops_city on public.shops (city)';
    execute 'create index if not exists idx_shops_state on public.shops (state)';
    execute 'create index if not exists idx_shops_city_lower on public.shops (lower(city))';
  end if;

  if to_regclass('public.barbershops') is not null then
    execute 'create index if not exists idx_barbershops_city on public.barbershops (city)';
    execute 'create index if not exists idx_barbershops_state on public.barbershops (state)';
    execute 'create index if not exists idx_barbershops_city_lower on public.barbershops (lower(city))';
  end if;
end;
$$;
